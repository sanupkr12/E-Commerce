...
function readFileDataCallback(results){
    ...
    if(fields.length != 2)
    {
        let errorHtml = "<p>Errors:</p><p class='text-danger'>Incorrect Number of fields</p>";
        handleError(errorHtml);
        return;
    }
    if(fields[0]!= 'sku_id' || fields[1]!= 'quantity'){
        let errorHtml = "<p>Errors:</p><p class='text-danger'>Incorrect headers</p>";
        handleError(errorHtml);
        return;
    }
    let data = results.data;
    if(data.length == 0){
        let errorHtml = "<p>Errors:</p><p class='text-danger'>Empty file</p>";
        handleError(errorHtml);
        return;
    }
    if(data.length > 10){
        let errorHtml = "<p>Errors:</p><p class='text-danger'>Number of rows exceeds limit(10)</p>";
        handleError(errorHtml);
        return;
    }
    validOrderItems = [];
    ...
    .then((json)=>{
        ...
        $("#order-list").find("tbody")[0].innerHTML = "";
